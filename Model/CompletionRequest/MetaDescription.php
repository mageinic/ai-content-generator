...
/**
 * Model CompletionRequest Class MetaDescription
 */
    ...
    public function getJsConfig(): ?array
    {
        ...
        if ($fullActionName == "catalog_product_edit" || $fullActionName == "catalog_product_new") {
            ...
            return [
                ...
                'container' => 'category_form.category_form.search_engine_optimization.seo_group_description',
                'target_field' =>
                    'category_form.category_form.search_engine_optimization.seo_group_description.meta_description',
                'component' => 'MageINIC_AIContentGenerator/js/generate_button_renderer'
            ];
        } else {
            return [];
    ...
    public function getApiPayload(array $text): array
    {
        ...
        if ($generatorType == "category") {
            $promptTxt = $text['custom_prompt'] === "true" ?
                $text['prompt'] :
                sprintf(
                    'Write seo meta description for "%s" category. Taking care of seo meta description character limit.
                Do not include half completed sentences.',
                    $text['prompt']
                );
        }
        if ($generatorType == "product") {
            $promptTxt = $text['custom_prompt'] === "true" ?
                $text['prompt'] :
                sprintf(
                    'Write seo meta description for "%s". Taking care of seo meta description character limit.
                Do not include half completed sentences.',
                    $text['prompt']
                );
        }
