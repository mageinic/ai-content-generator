...
/**
 * Model CompletionRequest Class MetaTitle
 */
class MetaTitle extends AbstractCompletion implements CompletionRequestInterface
{
    ...
    public function getJsConfig(): ?array
    {
        ...
        if ($fullActionName == "catalog_product_edit" || $fullActionName == "catalog_product_new") {
            ...
            return [
                'attribute_label' => 'Meta Title',
                'container' => 'category_form.category_form.search_engine_optimization.seo_group_title',
                'target_field' => 'category_form.category_form.search_engine_optimization.seo_group_title.meta_title',
                'component' => 'MageINIC_AIContentGenerator/js/generate_button_renderer'
            ];
        } else {
            return [];
    ...
    public function getApiPayload(array $text): array
    {
        ...
        if ($generatorType == "category") {
            $promptTxt = $text['custom_prompt'] === "true" ?
                $text['prompt'] :
                sprintf(
                    'Write a meta title for "%s" category.',
                    $text['prompt']
                );
        }
        if ($generatorType == "product") {
            $promptTxt = $text['custom_prompt'] === "true" ?
                $text['prompt'] :
                sprintf(
                    'Write a meta title for "%s" product.',
                    $text['prompt']
                );
        }
